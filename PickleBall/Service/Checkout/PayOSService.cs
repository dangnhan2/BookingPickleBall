...
namespace PickleBall.Service.Checkout
{
    public class PayOSService : IPayOSService
    {
        ...
        public async Task<dynamic> CreatePaymentLink(List<ItemData> items, int amount, int orderCode, Guid partnerId)
        {
            var partner = await _unitOfWorks.User.GetById(partnerId);

            if (partner == null)
                throw new KeyNotFoundException("Không tìm thấy partner");

            PayOS payOS = new PayOS(
                partner.PayOSClientId,
                partner.PayOSApiKey,
                partner.PayOSCheckSumKey);
        ...
        public async Task<string> ConfirmPayOSWebHook(Guid partnerId)
        {
            var partner = await _unitOfWorks.User.GetById(partnerId);
